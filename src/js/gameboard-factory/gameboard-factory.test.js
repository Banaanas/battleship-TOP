...
const countTrueCases = () =>
  gameboardTest.gameGrid.reduce((a, b) => a + (b === true), 0);
...
describe("gameboardTest.receiveAttack method - 2", () => {
  it("should not accept duplicated missedShot", () => {
    ...
    const previousAttackLengthBeforeDuplicated =
      gameboardTestEnemy.previousAttacks.length;
    // eslint-disable-next-line max-len
    gameboardTest.receiveAttack(96, gameboardTestEnemy); // Same duplicate strike - Duplicate missed shot
    expect(gameboardTestEnemy.previousAttacks.length).toBe(
      previousAttackLengthBeforeDuplicated,
    );
  });
});
...
describe("gameboardTest.receiveAttack method - 3", () => {
  it("should not accept duplicated damage", () => {
    ...
    // eslint-disable-next-line max-len
    expect(gameboardTestEnemy.previousAttacks.length).not.toBe(
      previousAttackLengthBeforeDuplicated,
    );
    // eslint-disable-next-line max-len
    expect(gameboardTestEnemy.previousAttacks.length).toBe(
      previousAttackLengthBeforeDuplicated + 1,
    );
  });
});
