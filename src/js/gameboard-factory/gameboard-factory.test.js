...
describe("gameGrid Length", () => {
  test("if gameGrid length is 100", () => {
    expect(gameboardTest.gameGrid.length)
      .toBe(100);
  });
});
...
describe("gameboardTest.receiveAttack method", () => {
  it("determines whether or not the attack hit a ship", () => {
    ...
    expect(gameboardTest.allShips[2].damages)
      .toContain(95); // 1 damage to borderPatrol
    expect(gameboardTest.missedShots.length)
      .toBe(0); // No missed shots
    expect(gameboardTest.missedShots)
      .not
      .toContain(95); // No missed shots

    gameboardTest.receiveAttack(75, gameboardTestEnemy); // 1 missed shot
    expect(gameboardTest.allShips[0].damages.length)
      .toBe(0); // 0 new damage to carrier
    expect(gameboardTest.allShips[1].damages.length)
      .toBe(0); // 0 damage to destroyer
    // eslint-disable-next-line max-len
    expect(gameboardTest.allShips[2].damages.length)
      .toBe(1); // 0 new damage to patrolBoat (still the precedent)
    expect(gameboardTest.missedShots.length)
      .toBe(1); // No missed shots
    expect(gameboardTest.missedShots)
      .toContain(75); // No missed shots

    gameboardTest.receiveAttack(4, gameboardTestEnemy); // 1 damage
    expect(gameboardTest.allShips[0].damages.length)
      .toBe(1); // 1 new damage to carrier
    expect(gameboardTest.allShips[0].damages)
      .toContain(4); // 1 new damage to carrier

    gameboardTest.receiveAttack(72, gameboardTestEnemy); // 1 damage
    expect(gameboardTest.allShips[1].damages.length)
      .toBe(1); // 1 new damage to destroyer
    expect(gameboardTest.allShips[1].damages)
      .toContain(72); // 1 new damage to destroyer
    gameboardTest.receiveAttack(96, gameboardTestEnemy); // 1 damage

    // eslint-disable-next-line max-len
    expect(gameboardTest.allShips[2].damages.length)
      .toBe(2); // 1 new damage to patrolBoat (+ 1 before)
    expect(gameboardTest.allShips[2].damages)
      .toContain(95); // 2 damages to patrolBoat (95 - 96)
    expect(gameboardTest.allShips[2].damages)
      .toContain(96); // 2 damages to patrolBoat (95 - 96)
    ...
    expect(gameboardTest.missedShots.length)
      .toBe(5); // 4 + 1 missed shots
    expect(gameboardTest.missedShots)
      .toEqual([75, 35, 62, 18, 86]); // 5 missed shots
  });
});
...
describe("gameboardTest.allShipsSunk method", () => {
  test("if the game does NOT end while one players ships have not ALL been sunk", () => {
    expect(gameboardTest.allShipsSunk())
      .toBe(false);
  });
