...
const asyncComputerAttack = async (
  humanGameboard,
  computerGameboard,
  humanGameboardCases,
) => {
...
const gameController = (humanGameboard, computerGameboard) => {
  ...
  const computerGameboardCases = document.querySelectorAll(
  ...
  computerGameboardCases.forEach((computerGridCase, index) => {
    computerGridCase.addEventListener("click", () => {
      ...
      if (
        humanGameboard.battleDefeat === true ||
        computerGameboard.battleDefeat === true
      ) {
        return;
      }
      // Human Navy Attack
      ...
      if (computerGameboard.duplicateStrike === true) {
        return;
      }
