...
describe("gameboardTest..randomCoords method", () => {
  ...
  it("should generate X (shipLength) numbers with a difference of 10 between each if VERTICAL MOVE", () => {
    ...
    expect(checkDifference(verticalCarrierCoords)).toBe(true);
    expect(checkDifference(verticalBattleshipCoords)).toBe(true);
    expect(checkDifference(verticalDestroyerCoords)).toBe(true);
    expect(checkDifference(verticalSubmarineCoords)).toBe(true);
    expect(checkDifference(verticalPatrolBoatCoords)).toBe(true);

    expect(checkDifference([10, 11, 12, 13, 14])).toBe(false);
  });
